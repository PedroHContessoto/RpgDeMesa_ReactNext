...
function Entrada() {
  ...
  function handleFormSubmit(event) {
    ...
    const hasUnlocked = localStorage.getItem('hasUnlockedlevel1');
    if (hasUnlocked) {
      alert('Você já ganhou os pontos desse nivel');
      return;
    }
    if (code === '1234') { // Substitua '1234' pelo código correto
      const gainedGold = 10; // Defina aqui a quantidade de gold ganha pelo usuário
      const gainedXP = 50; // Defina aqui a quantidade de XP ganha pelo usuário
      ...
      localStorage.setItem('hasUnlockedlevel1', true);
      setIsUnlocked(true);
  ...
  return (
    <main className={styles.body}>
      <BrosSideBar />
      <h1 className={styles.h1}>Bem-vindo à Entrada</h1>
      {isUnlocked ? (
        <div>
